...
const chartColors = [
  "#4f46e5", "#10b981", "#f59e0b", "#ef4444", "#8b5cf6", "#14b8a6",
];
...
export default function Home() {
  ...
  const monthlyComparisonOptions = useMemo(() => ({
    ...
    legend: { data: years.map((year) => String(year)), top: "5%" },
    grid: { left: "10%", right: "10%", bottom: "12%", top: "18%" },
    xAxis: { type: "category", data: monthlyComparisonData.map((item) => item.month) },
    yAxis: { type: "value", name: "Tonnes" },
  ...
  const growthOptions = useMemo(() => ({
    ...
    legend: { data: ["Current year", "Growth %"], top: "5%" },
    grid: { left: "10%", right: "10%", bottom: "12%", top: "18%" },
    xAxis: { type: "category", data: growthData.map((item) => item.month) },
    yAxis: [
  ...
  const cumulativeComparisonOptions = useMemo(() => ({
    ...
    legend: { data: years.map((year) => String(year)), top: "5%" },
    grid: { left: "10%", right: "10%", bottom: "12%", top: "18%" },
    xAxis: { type: "category", data: cumulativeComparisonData.map((item) => item.month) },
    yAxis: { type: "value", name: "Tonnes" },
    series: years.map((year, index) => ({
      name: String(year),
      type: "line",
